	...
	export default {
		...
		methods: {
			...
			getPointTransactionDetails() {
				...
				axios.get('/tbcgoldadmin/api/transactions/cashout/matrix-2/details/' + this.cashout_transaction_id).then((resp) => {
					...
					if (resp.data.status == 'ok') {
						...
						// member
						this.member.alias = resp.data.member.user.alias_name;
						this.member.fullname = resp.data.member.user_detail.fullname;
						this.member.referral_points = parseInt(resp.data.member.level_two_user_referral_points);
						this.member.left_points = parseInt(resp.data.member.level_two_user_left_points);
						this.member.right_points = parseInt(resp.data.member.level_two_user_right_points);
						this.member.btc_wallet = resp.data.member.user_detail.btc_wallet;
						this.member.current_cashout_pairing_level = parseInt(resp.data.member.level_two_user_current_cashout_pairing_level);
						this.member.previous_cashout_pairing_level = parseInt(resp.data.member.level_two_user_previous_cashout_pairing_level);
						this.member.commission_balance = parseFloat(resp.data.member.level_two_user_commission_balance);
						this.member.upgrading_total = parseFloat(resp.data.member.level_two_user_upgrading_total);
			...
			confirmPointTransaction() {
				...
				axios.post('/tbcgoldadmin/api/transactions/cashout/matrix-2/confirm', {
			...
			updateTransactionHash(id) {
				...
				axios.post('/tbcgoldadmin/api/transactions/cashout/matrix-2/hash/update', {
			...
			viewTransactionListing() {
				return '/tbcgoldadmin/transactions/cashout/matrix-2';
			},
