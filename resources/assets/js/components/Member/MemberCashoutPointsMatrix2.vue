...
<div>
	...
	<div class="ui cashout-points-matrix-2-modal modal mini">
		<div class="header">
			Confirm Cash Out
		</div>
		...
		methods: {
			getMemberDetails() {
				...
				axios.get('/member/api/account-details').then((resp) => {
					...
					this.points = resp.data.user.level_two_user.level_two_user_left_points + resp.data.user.level_two_user.level_two_user_right_points;
					this.left_points = resp.data.user.level_two_user.level_two_user_left_points;
					this.right_points = resp.data.user.level_two_user.level_two_user_right_points;
					this.referral_points = resp.data.user.level_two_user.level_two_user_referral_points;
			...
			showConfirmationModal() {
				this.$validator.validateAll().then((result) => {
					if (result) {
						$('.ui.cashout-points-matrix-2-modal.modal').modal('show');
					} else {
			...
			closeConfirmTransactionModal() {
				$('.ui.cashout-points-matrix-2-modal.modal').modal('hide');
			},
			...
			cashoutPoints() {
				this.$validator.validateAll().then((result) => {
					if (result) {
						...
						axios.post('/member/api/cashout/matrix-2', {
