		...
		<div class="ui raised segment">
			...
			<div class="ui icon message" v-if="showSuccess">
			  ...
			  <div class="content">
			    ...
			    <p>
			    	Your membership is on process now. Please be patient on waiting as we are validating the details you have submitted.
			    </p>
			  </div>
			</div>
				...
				<form class="ui form" @submit.prevent="submitForm()">
					<div v-show="step1">
						<div class="field" :class="{ error: errors.has('email') }">
							...
							<div class="ui right icon input" :class="{loading: checkingEmail}">
							  <input 
									type="text" 
									name="email" 
						...
						<div class="field" :class="{ error: errors.has('alias') }">
							...
							<div class="ui right icon input" :class="{loading: checkingSponsorAlias}">
								<input 
									type="text" 
									name="alias" 
									placeholder="XXXXXXXXXXX" 
									...
									data-vv-as="Direct Upline Alias"
								/>
							</div>
						...
						<div class="field" :class="{ error: errors.has('direct_sponsor') }">
							<label>Your Direct Sponsor(alias)</label>
							<div class="ui right icon input" :class="{loading: checkingSponsorAlias}">
								<input 
									type="text" 
									name="direct_sponsor" 
									...
									v-validate="'required|sponsor_exists'"
									data-vv-as="Direct Sponsor Alias"
								/>
							</div>
						...
						<div class="field" :class="{ error: errors.has('sponsor_alias') }">
							<label>Your Direct Upline(alias) <small>Note: direct upline must not have 2 or more than direct recruit. But you can place him as your direct sponsor.</small></label>
							<div class="ui right icon input" :class="{loading: checkingSponsorAlias}">
								<input 
									type="text" 
									name="sponsor_alias" 
									placeholder="XXXXXXXXXXX" 
									v-model="sponsor_alias" 
									v-validate="'required|sponsor_exists2'"
									data-vv-as="Direct Upline Alias"
								/>
							</div>
							
							<div class="ui pointing red basic label" v-if="errors.has('sponsor_alias')">
								{{ errors.first('sponsor_alias') }}
							</div>
						</div>
...
<script>
	...
	export default {
		name: 'member-auth-sign-up-with-sponsor',
		props: {
			btc_value: {
				type: String,
				required: true
		...
		created () {
			...
			const isSponsorAliasExists = (value) => {
				this.checkingSponsorAlias = true;
			  return axios.post('/member/api/validate-sponsor-alias', { sponsor_alias: value }).then((resp) => {
			    // Notice that we return an object containing both a valid property and a data property.
			...
			// The messages getter may also accept a third parameter that includes the data we returned earlier.
			Validator.extend('sponsor_exists', {
			  validate: isSponsorAliasExists,
			  getMessage: (field, params, data) => {
			    return data.message;
			  }
			});
			...
			submitMemberData() {
				...
				this.$validator.validateAll().then((result) => {
					if (result) {
						...
						formData.append('sponsor_alias', this.sponsor_alias);
						formData.append('direct_sponsor', this.direct_sponsor);
						...
						axios.post('/member/api/sign-up-with-sponsor', formData).then((resp) => {
							console.log(resp);
